    ...
    $.fn[pluginName] = function(options) {
        ...
        return this.each(function() {
            ...
            $this.on('keydown.' + pluginName, function(e) {
                ...
                if (e.keyCode === 9) {
                    //page-menu-close-button-dummy

                    var tabbables = $this.find(':focusable'),
                        //first = tabbables.filter(':first'),
                        first = $(options.first),
                        last = tabbables.filter(':last'),
                        focusedElement = $(e.target),
                        isFirstInFocus = (first.get(0) === focusedElement.get(0)) || (first.attr("class") === focusedElement.get(0).className) ,
